...
/* show tables */
if ($state == 'INPROGRESS') {
	$sql = "select * from `rs_super_tasks` WHERE `owner` = '$user' and `state` = '$state'";
} else {
	$sql = "select * from `rs_super_tasks` WHERE `owner` = '$user'";
}
...
if ($result) {
	echo "<h2> Your tasks in state - $state : $result->num_rows </h2>"; 
	echo '<table>';
